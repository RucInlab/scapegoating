function [shortestPaths, totalCosts] = kShortestPath(netCostMatrix, monitors,links, k_paths,independent_paths)
...
%==============================================================
k=0;
combination = nchoosek(monitors,2);
n = length(combination);
for combination_id = 1:n
    if combination(combination_id,1) > size(netCostMatrix,1) || combination(combination_id,2) > size(netCostMatrix,1)
        ...
        %---------------------INITIALIZATION---------------------
        %k=1;
        [path cost] = dijkstra(netCostMatrix, combination(combination_id,1), combination(combination_id,2));
        ...
        if isempty(path)
            ...
            count = 0;
            %--------------------------------------------------------
            while (count < k_paths-1   &&   size_X ~= 0 )
                %remove P from X
                for i=1:length(X)
                    if  X{i}{1} == current_P
                        size_X = size_X - 1;
                ...
                for index_dev_vertex= w_index_in_path: length(P_) - 1   %index_dev_vertex is index in P_ of deviation vertex
                    ...
                    for j = 1: length(SP_sameSubPath)
                        if length(SP_sameSubPath{j}) > index_dev_vertex
                    ...
                    [dev_p c] = dijkstra(temp_netCostMatrix, P_(index_dev_vertex), combination(combination_id,2));
                    if ~isempty(dev_p)
                ...
                if size_X > 0
                    ...
                    current_P = shortestX;
                    %******
%                         if length(intersect(P{current_P,1},source)) == 1 && length(intersect(P{current_P,1},destination)) == 1
%                             k = k+1;
